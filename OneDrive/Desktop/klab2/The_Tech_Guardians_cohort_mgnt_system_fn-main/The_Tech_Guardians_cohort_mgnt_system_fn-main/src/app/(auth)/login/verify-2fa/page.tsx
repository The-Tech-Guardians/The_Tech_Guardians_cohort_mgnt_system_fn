...
export default function TwoFAPage() {
  ...
  useEffect(() => {
    ...
    // Auto-fill development 2FA code if available
    const devCode = localStorage.getItem('dev_2fa_code');
    if (devCode && devCode.length === 6) {
      console.log('🔓 Auto-filling development 2FA code:', devCode);
      setOtp(devCode.split(''));
      // Auto-submit after a short delay
      setTimeout(() => verifyOTP(devCode), 500);
    }
  }, [router]);
  ...
  const verifyOTP = async (code: string) => {
    ...
    try {
      ...
      if (response.message === 'Login successful' || response.success) {
        ...
        // Clear development 2FA code from localStorage

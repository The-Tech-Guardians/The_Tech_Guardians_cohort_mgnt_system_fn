...
function LessonCard({ lesson, modules, courses, onEdit, onDelete }: LessonCardProps) {
  const moduleName = getModuleName(lesson.moduleId, modules);
  const courseName = getCourseName(modules.find(m => m.id === lesson.moduleId)?.courseId || "", courses);
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-5 hover:shadow-md transition-all">
      ...
      {isVideo && lesson.contentUrl && (
        <div className="mb-3">
          <video 
            controls 
            className="w-full rounded-lg shadow-sm border max-h-32"
            preload="metadata"
          >
...
function LessonRow({ lesson, modules, courses, onEdit, onDelete }: LessonRowProps) {
  const moduleName = getModuleName(lesson.moduleId, modules);
  const courseName = getCourseName(modules.find(m => m.id === lesson.moduleId)?.courseId || "", courses);
  const isVideo = lesson.contentType === "video";
  ...
  return (
    <tr className="hover:bg-gray-50">
      ...
      <td className="px-6 py-4 text-xs text-gray-400">{courseName}</td>

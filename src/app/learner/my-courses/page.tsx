...
export default function LearnerMyCoursesPage() {
  ...
  if (loading) {
    return (
      <div
        className={`transition-all duration-300 space-y-6 ${collapsed ? "mx-4" : "max-w-6xl mx-auto"}`}
      >
        ...
        <div className="grid md:grid-cols-3 gap-6 p-4 bg-gray-50 rounded-2xl animate-pulse">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-white rounded-2xl border border-gray-200 shadow-sm h-80"
            >
    ...
    >
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {cohort
              ? `Courses in ${cohort.name}`
              : "Join a cohort to get started"}
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          {["all", "in-progress", "not-started"].map((f) => (
            <button
              key={f}
              onClick={() =>
                setFilter(f as "all" | "in-progress" | "not-started")
              }
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-sm font-medium text-gray-600">
                Students in{" "}
                <span className="font-bold text-gray-900">{cohort.name}</span>
              </p>
      ...
      {courses.length === 0 ? (
        <div className="text-center py-20 px-8">
          <div className="w-24 h-24 bg-gradient-to-r from-blue-600 to-cyan-500 p-6 rounded-2xl mx-auto mb-6 shadow-2xl">
          ...
          <Link
            href="/learner/cohorts"
            className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-bold rounded-2xl shadow-2xl hover:shadow-3xl hover:from-indigo-700 transition-all text-lg"
          >
            Browse Cohorts
          </Link>
        </div>
          ...
          {filtered.map((course) => (
            ...
            >
              ...
              <div className="p-6 space-y-4">
                <div>
                  <h3
                    className="font-black text-xl text-gray-900 leading-tight group-hover:text-indigo-600 transition-colors line-clamp-2"
                    style={{ fontFamily: "'Bricolage Grotesque', sans-serif" }}
                  >
                    {course.title}
                  </h3>
                  <p className="text-sm text-gray-500 mt-1">
                    by {course.instructor || "Tech Guardians Instructor"}
                  </p>
                </div>
                ...
                <div className="">
                  <div className=" mt-1 text-gray-900 font-medium text-xs">
                    Progress
                  </div>
